...
try {
  ...
  const symbolGearNumberMap = new Map<string, number[]>();

  [...symbolLocationMap].forEach((symbolLocation) => {
    if (symbolLocation[1] === "*") {
      const symbolLocationCoords = symbolLocation[0].split(",");
      symbolGearNumberMap.set(
        `${symbolLocationCoords[0]},${symbolLocationCoords[1]}`,
        []
      );
    }
  });
  ...
  for (
    ...
    [
      ...
      let hasPotentialGear = symbolGearNumberMap.has(rowColStr);
      if (hasPotentialGear) {
        symbolGearNumberMap.set(rowColStr, [
          ...(symbolGearNumberMap.get(rowColStr) || []),
          number,
        ]);
      }
    });
  ...
  //   console.log(symbolGearNumberMap);

  const attachedNumbersToPotentialGears = [...symbolGearNumberMap].map(
    (a) => a[1]
  );

  const test = attachedNumbersToPotentialGears.reduce(
    (acc, attachedNumbersToPotentialGear) => {
      if (attachedNumbersToPotentialGear.length !== 2) return acc;
      //   console.log(attachedNumbersToPotentialGear);
      return [
        ...acc,
        attachedNumbersToPotentialGear[0] * attachedNumbersToPotentialGear[1],
      ];

      //   return [
      //     ...acc,
      //     attachedNumbersToPotentialGear.reduce((acc, num) => acc * num, 0),
      //   ];
    },
    [] as number[]
  );
  ...
  console.log(test);
  console.log(test.reduce((acc, curr) => acc + curr, 0));
