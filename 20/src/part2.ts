    ...
    const destinationModules = destinationModuleNames.reduce(
      (acc, destinationModuleName) => {
        ...
        if (destinationModule) return [...acc, destinationModule];
          ...
          const newOutputModule = new OutputModule(destinationModuleName);
          if (destinationModuleName === "rx") {
            newOutputModule.endOnLow = true;
          }
          moduleMap.set(destinationModuleName, newOutputModule);
...
const pushTheButton = async (buttonModule: Module, buttonPressNum: number) => {
  let lowPulses = 0,
    highPulses = 0;
...
const main = async (input: string) => {
  ...
  for (
    ...
    buttonPressIndex < 1000000000;
    buttonPressIndex++
  ) {
    const { highPulses, lowPulses } = await pushTheButton(
      buttonModule,
      buttonPressIndex + 1
    );
